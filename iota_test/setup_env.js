...
async function main() {
    ...
    } else if (fs.existsSync('test_config.example.json')) {
        // If no config, try to load PACKAGE_ID from example
        console.log('Loading defaults from test_config.example.json...');
        const example = JSON.parse(fs.readFileSync('test_config.example.json', 'utf8'));
        if (example.PACKAGE_ID && example.PACKAGE_ID !== "0x...") {
            config.PACKAGE_ID = example.PACKAGE_ID;
            console.log(`Using Shared Package ID: ${config.PACKAGE_ID}`);
        }
        if (example.NODE_URL) config.NODE_URL = example.NODE_URL;
        if (example.FAUCET_URL) config.FAUCET_URL = example.FAUCET_URL;
    }

    config.NODE_URL = config.NODE_URL || DEFAULT_NODE_URL;
    config.FAUCET_URL = config.FAUCET_URL || DEFAULT_FAUCET_URL;

    // config.PACKAGE_ID = ""; // Removed to prevent forced re-publish
